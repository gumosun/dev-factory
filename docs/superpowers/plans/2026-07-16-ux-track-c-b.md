...
head -6 agents/visual-reviewer.md && \
  grep -c 'VERDICT: PASS\|VERDICT: CHANGES_REQUIRED\|VERDICT: SKIPPED' agents/visual-reviewer.md
```
Expected: frontmatter 有 `name: visual-reviewer` / `tools: Read, Write, Edit, Bash, Glob` / `model: opus`；grep 計數 ≥ 3
