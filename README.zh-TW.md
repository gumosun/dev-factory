...
> 此處 `vendor/token-lens/` 是被鋪進專案的 **runtime 快照**（觀測＋路由腳本；見 `vendor/token-lens/UPSTREAM.md`）——改請走上游工作副本、用 `sync-from-upstream.sh` 同步，別在此直接編輯。
...
├── vendor/token-lens/     可選成本觀測＋自我學習路由層（ledger/quality/radar/router/retro_optimize + 政策/情報；runtime 快照，鋪到專案 .claude/token-lens/；見 UPSTREAM.md）
