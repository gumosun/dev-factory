...
- **`/sprint`（建造）**：給定目標與 backlog，orchestrator 主持 PM → UX → 架構（設計+拆解一棒）→ 一致性 → 開發 → 驗證關 → 收尾的 sprint，角色之間透過檔案交接，gate 不過自動退回重跑，只在 sprint 收尾或卡關時找你。**流程隨「治理 profile」（lean/standard/max）伸縮**——這是控制時間與 token 成本的主鈕：lean 把驗證關合併成單一 reviewer、一致性折進架構自檢；max 全拆專家棒。
...
```
dev-factory/
├── agents/          13 個角色 subagent（建造10：含合併驗證關 reviewer + discovery 3: explorer/critic/shaper）
