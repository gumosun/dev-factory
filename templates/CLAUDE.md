...
pm → （ux-designer，有可見面才跑）→ architect（設計+拆解一棒融合）→ （consistency-reviewer，standard 有可見面才跑）→ developer → 驗證關（lean=單一 reviewer／standard=QA+reviewer／max=QA+security+drift）→ pm 收尾（瘦身）→ retro 回顧。
...
- **只有這幾種情況才暫停找我**：
  ...
  (f) **本專案首次開跑、治理 profile 未設** → 建議一個 profile 讓我選（每專案一次）。
- 其餘一律自己往前推進。
