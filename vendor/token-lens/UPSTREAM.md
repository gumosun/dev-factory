...
- **原始檔在上游工作副本**（`sync-from-upstream.sh` 預設 `~/Desktop/token-lens`）。**改請改上游，再跑 `./sync-from-upstream.sh` 同步回來，別在這裡直接編輯。**
- 這裡只放執行期需要的檔（ledger / quality / radar / router / retro_optimize / policy / intel / seed），讓安裝後的專案自足。
