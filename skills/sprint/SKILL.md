...
3. **UX 強度判定（同一次 HUMAN GATE，每專案問一次）**：讀 `docs/PROJECT_GOAL.md` 是否已有「UX 強度：light / full」欄位。
   - **已設** → 沿用，不再問。
   - **未設，且 profile 也未設**（全新專案）→ **與 profile 同一次問完**（不要分兩次打擾使用者）。
   - **未設，但 profile 已設**（本框架加入 UX 強度之前就建立的既有專案）→ **逕用 `light` 往下跑，不打斷**（`light` ＝ 這些專案一直以來的行為，不會有任何改變），並在**本輪收尾摘要**順帶告訴使用者：「本專案的 UX 強度未設、已用預設 `light`；若這個專案的 UI 品質是重點，可在 `PROJECT_GOAL.md` 標 `full` 啟用設計系統約束 + 視覺關。」——**不要為了問這件事而中斷 sprint**（不符合自主邊界的停下條件）。
   - 建議哪一個時，依專案是否以 UI 為主要價值來源判斷：
