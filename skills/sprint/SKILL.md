...
3. **決定 N 與續跑判定**：N 以 `docs/sprints/` 中 `sprint-<N>.md` **主檔**的最大編號為準（忽略 `_TEMPLATE.md` 與 `-tasks/-dev/-review/-qa/-security/-drift` 等衍生檔）。讀最新主檔的「執行狀態」區塊：若有未勾階段 → 這是**續跑**，從第一個未完成階段接續，各 gate 退回計數沿用區塊記錄；若全部完成 → 開新 sprint N+1。
4. **開分支**：若專案是 git repo，建立並切到 `sprint-<N>` 分支（已存在就直接 checkout 沿用）。合併回主分支由收尾 human gate 決定，你**不**自行 merge。
5. 若 backlog 為空且使用者不想跑 discovery → 告訴使用者沒有待辦，停。
...
- **迴圈計數規則**：各 gate 的退回計數**獨立且不重置**。另設 **sprint 總修復預算 6 次**：任何 gate（一致性/驗證關各區塊）的退回都累計，達 6 次即停下升級使用者（防 gate 之間互彈空轉）。所有計數記在「執行狀態」區塊，不憑記憶。
- **只有這幾種情況才停下找使用者**：
...
## 給使用者的收尾摘要格式
做了什麼 / 驗收標準達成情況 / 驗證關結果（功能・資安・飄移） / 未竟事項 / **retro 學到什麼（新增 LESSONS、待核可改善提案 PROJECT-local 與 FRAMEWORK 各幾條）** / 下一個 sprint 建議 / **分支 `sprint-<N>` 是否建議合併回主分支**（合併與否由使用者決定）。

## Gate 報告檔後置驗證（R-15-1，2026-07-13 核可）
每派一個 gate（一致性/合併 reviewer/QA/資安/飄移）並收到回報後，orchestrator **必須先確認對應報告檔已存在**（Read/ls 對應 docs 路徑：`-review.md` / `-consistency.md` / `-qa.md` / `-security.md` / `-drift.md`）再推進下一階段。若缺檔：要求該 agent 補寫；agent 稱限制無法補寫時，orchestrator 逐字代錄為後備（檔頭標明「orchestrator 代錄、逐字保存」）。**不得在報告檔缺席下推進下一階段。**
